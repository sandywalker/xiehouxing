...
use App\WxPay\PayResults;
use App\WxPay\Report;
use App\WxPay\WxData;
use App\WxPay\WxException;
...
class PayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(PayConfig::NOTIFY_URL);//异步通知url
		}
	...
	{
		//获取通知的数据
		$xml = Request::getContent(); //$GLOBALS['HTTP_RAW_POST_DATA'];
		//如果返回成功则验证签名
		try {
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, base_path().PayConfig::SSLCERT_PATH);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, base_path().PayConfig::SSLKEY_PATH);
		}
		//post提交方式
